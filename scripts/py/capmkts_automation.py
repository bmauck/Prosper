...
def write_cumul_files(investors, date1, date2, reports):
        ...
        for i in investors:
            ...
            for day in dates:
                ...
                engine = sqlalchemy.create_engine(
                        'mssql+pyodbc://@dbrpt/{db}?driver=SQL+Server+Native+Client+11.0'.format(db=db))
                ...
                cumul_file.to_csv(fld+str(i)+'cumul_remits_'+str(date1)+'_to_'+date2+'.csv')
            print('Writing Files...')
...
def write_files(investors, date1, date2, reports):
...
def explore_file(investor, date1, date2, report):
    ...
    if report == 'Remit':
        query = 'exec Report_DailyLenderPacket_{} {}, {}'.format(report,investor,d2)
    else:
        query = 'exec Report_DailyLenderPacket_{} {}, {}, {}'.format(report,investor,d1,d2)
    db = 'ReportingProgrammability'
    engine = sqlalchemy.create_engine(
...
def generate_agreements(AgreementTypeID, loans):
    ...
    connection = engine.connect()
    print('Query...')
    p = pd.read_sql_query(query.format(loans, AgreementTypeID, AgreementTypeID), connection, index_col='Loan')
    connection.close()
    
    print('Writing Files...')
    for index, row in p.iterrows():
        if row.IsCorrectedAgreement is True:
            file = dir+'{} {} (Corrected)'.format(index, row.Title)
            ...
            file = dir+'{} {}'.format(index, row.Title)
...
def generate_tape(investor, date):
    ...
    engine = sqlalchemy.create_engine('mssql+pyodbc://@dbrpt/{db}?driver=SQL+Server+Native+Client+11.0'.format(db=db))
    connection = engine.connect()
    query = query.format(date, date, investor)
    tape = pd.read_sql_query(query, connection)
    connection.close()
    print('Done')
    return tape
...
def elig(tape):
    ...
    if tape['DaysPastDue'] > 30:
        ...
        return 0
    elif tape['SettlementStatus'] != 0:
        return 0
    elif tape['ExtensionStatus'] != 0:
        return 0
    elif tape['IsSCRA'] == True:
        return 0 
    elif (tape['LoanStatusDescription'] == 'COMPLETED') & (tape['InProcessPrincipalPayments'] == 0):
        return 0
        ...
        return 0 
    elif (tape['BorrowerState'] == 'GA') & (tape['OriginalInvestment'] <= 3000):
        return 0
    elif (tape['BorrowerState'] == 'CO') & (tape['BorrowerAPR'] > 0.21):
        return 0
    elif (tape['BorrowerState'] == 'NY') & (tape['BorrowerAPR'] > 0.16):
        return 0
    elif (tape['BorrowerState'] == 'CT') & (tape['BorrowerAPR'] > 0.12):
        return 0
    elif (tape['BorrowerState'] == 'VT') & (tape['BorrowerAPR'] > 0.12):
        return 0
    else:
        return 1
...
def calc_tape_metrics(tape):
    d = {}
    d['Principal Balance'] = tape['PrincipalBalance'].sum()
    d['Wtd Avg Maturity'] = np.average(tape['AgeInMonths'], weights=tape['PrincipalBalance'])
    d['Wtd Avg Cpn'] = np.average(tape['BorrowerRate'], weights=tape['PrincipalBalance'])
    d['Wtd Avg FICO'] = np.average(tape['FICOScorePt'], weights=tape['PrincipalBalance'])
    d['Wtd Avg Income'] = (np.average(tape['MonthlyIncome'], weights=tape['PrincipalBalance']))*12
    d['Wtd Avg Debt'] = (np.average(tape['MonthlyDebt'], weights=tape['PrincipalBalance']))*12
    d['Wtd Avg DTI'] = np.average(tape['DTIwProsperLoan'], weights=tape['PrincipalBalance'])
    return pd.Series(d, index=['Principal Balance', 'Wtd Avg Cpn', 'Wtd Avg Maturity', 'Wtd Avg FICO', 'Wtd Avg Income', 'Wtd Avg Debt', 
                               'Wtd Avg DTI'])

def gen_tape_metrics(deal_name, tape, label):
    
    tape_metrics = tape.groupby(['Term','ProsperRating']).apply(calc_tape_metrics)

    xlsx_filename = 'D:\\CapitalMarkets\\Output Files\\'+deal_name+' '+label+'.xlsx'
...
def generate_validation_file(investor, date, file_name):
    print('Querying...')
    query =  """
    DECLARE @CutoffMin	DATETIME	= '{}'
    DECLARE @CutoffMax	DATETIME	= '{}'
...
SELECT
	p.LoanNumber
	,'Obligor Name' = upper(lca.FirstName) + ' ' + upper(lca.LastName)
	,'Obligor State' = lca.BorrowerState
	,'BorrowerAPR' = lca.BorrowerAPR
	,'InterestRate' = p.BorrowerRate
	,'MaturityDate' = dateadd(month, p.term, p.OriginationDate)
	,'Monthly Payment Amount' = p.MonthlyPaymentAmount
	,'Origination Date' = p.OriginationDate
	,'OriginalAmountBorrowed' = lca.OriginalAmountBorrowed
	,'OriginationFeeAmount' = lca.OriginationFeeAmount
	,'TermMonths' = p.Term
	FROM
		#Positions p
	JOIN 
    ...
    connection.close()
    print('Done')
    tape.to_csv(dest_fld+file_name+'.csv')
    return tape
