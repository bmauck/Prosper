...
,end_cycle as (
  select
    OriginationQuarter
    ,min(llm.CycleCounter) as EndCycle
    from
      `DW.vloanlevelmonthly` llm
    where
      1=1
      and llm.ObservationMonth = (select * from date_params limit 1)
...
select
  ...
  ,ld.Term
	...
	,sum(MargPrinBK + MargPrinCO) + sum(case
                                       when SettlementStatus = 'settlecomp'
                                            and extract(month from SettlementEndDate) <= cast(ObservationMonth as int64)
    ...
    and parse_datetime('%E4Y%m', ld.ObservationMonth) >= (select MonthAcquired from loan_params limit 1)
  group by
    ld.OriginationQuarter
  	,ld.ProsperRating
	  ,ld.Term
